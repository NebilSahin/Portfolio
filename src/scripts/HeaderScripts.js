...
export const HeaderHover = () => {
    ...
    window.addEventListener(
        ...
        () => {
            headerStyle =
                header.currentStyle || window.getComputedStyle(header);
            windowExtrasSpace =
                parseInt(document.body.clientWidth) -
                parseInt(headerStyle.width);
        },
        true
    );
    ...
    function animate() {
        ...
        if (isAnimate) {
            offsetDistance =
                140 -
                ((mouseDelayed - windowExtrasSpace / 2) /
                    parseInt(headerStyle.width)) *
                    100;
...
export const PhotoMoveHover = (container, img, offset) => {
    ...
    function animate() {
        ...
        if (isAnimate) {
            let increaseX =
                (mouseDelayed - animatedElement.offsetLeft) / offsetAmount;
